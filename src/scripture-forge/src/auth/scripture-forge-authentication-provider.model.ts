...
import SecureSerialStorageManager from './secure-serial-storage-manager.model';
...
function createAuthorizationCodeAsyncVariable() {
  ...
  (async () => {
    try {
      ...
      logger.debug(
        `Authorization code async variable rejected. This may not be a problem. This happens the first time you log in and any time logging in is disrupted. ${e}`,
      );
    }
  })();
...
function getExpireTimeMs(expiresInSec: number): number {
  // Now plus the token lifetime converted from seconds to milliseconds
...
export default class ScriptureForgeAuthenticationProvider implements Dispose {
  private unsubscriberPromises: Promise<Unsubscriber | UnsubscriberAsync>[] = [];
  ...
  constructor(
    ...
    configuration: ServerConfigurationPresetNames | ServerConfiguration,
    private storageManager: SecureSerialStorageManager | undefined,
  ...
  async handleAuthCallback(searchParams: URLSearchParams): Promise<void> {
    // About to test this type to make sure it is correct
  ...
  async isLoggedIn(): Promise<boolean> {
    try {
      ...
      logger.debug(`Error trying to get access token to check if logged in: ${e}`);
    }
    return false;
  }
  ...
  async fetchWithAuthorization(url: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (response.status === 401) {
      try {
        ...
        throw new Error(`Error parsing 401 unauthorized error response: ${e}`);
      }
    }
    try {
  ...
  async dispose() {
    ...
    // TODO: Catch errors
    const unsubscribers = await Promise.all(this.unsubscriberPromises);
    const didEachSucceed = await Promise.all(unsubscribers.map((unsubscriber) => unsubscriber()));
    return didEachSucceed.every((didSucceed) => didSucceed);
  }
      ...
      if (this.storageManager) {
        ...
        if (isString(authTokensJSON)) {
          try {
            ...
            logger.warn(`Error parsing auth tokens from storage: ${e}`);
          }
        }
      }
    ...
    if (this.#authTokens) {
      ...
      if (refreshToken) {
        try {
          ...
          throw new Error(`Error refreshing access token: ${e}`);
        }
      }
    }
