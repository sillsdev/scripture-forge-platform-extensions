...
export type ProjectTableProps = {
  ...
  /** Callback fired when the project table detects the user has been logged out of SF */
  onDidLogOut?: () => void;
};
...
export default function ProjectTable({
  ...
  }, [onDidLogOut, refreshProjectsInfoListener]);
  ...
  const buildTableDraftSetupStateElement = (projectInfo: ProjectInfo) => {
    ...
    else if (connectionState === 'cannotAccessDrafts') statusText = 'No access';

    if (!statusText) {
      switch (draftSetupState) {
        case 'draftingNotAvailable':
          statusText = 'Drafting not available';
          break;
        case 'noFinishedDraft':
          statusText = isDraftGenerating ? 'Generating' : 'No drafts';
          break;
        case 'hasFinishedDraft':
          statusText = isDraftGenerating ? 'Generating' : 'Draft ready';
          break;
        default:
          throw new Error(
    ...
    switch (connectionState) {
      ...
      case 'cannotSetUp':
        return (
          <div className="tw-text-xs">
            Ask your administrator to connect this project to Scripture Forge.
          </div>
        );
      case 'canSetUp':
        return (
      ...
      case 'canJoin': {
        ...
        return (
          ...
          >
            {isJoining ? <Spinner /> : 'Join project'}
          </Button>
        );
      }
      case 'cannotAccessDrafts':
        return <div className="tw-text-xs">Ask your administrator for access to AI drafts.</div>;
      case 'connected': {
        const isDraftGenerating =
          projectInfo.draftInfo.currentlyGeneratingDraftStatus &&
          projectInfo.draftInfo.currentlyGeneratingDraftStatus.state !== 'COMPLETED';
        ...
        if (draftSetupState === 'hasFinishedDraft')
          return (
            <div className="tw-flex tw-flex-row tw-gap-1">
                      ...
                      decorations: {
                        headers: {
                          'slingshot-ai-header': {
                            ...
                            iconUrl:
                              'papi-extension://scriptureForge/assets/images/lucide-sparkles-0.378.0.svg',
                            descriptionMd: `This AI-generated pre-translation contains errors. [Open in Scripture Forge](${serverConfiguration.scriptureForge.domain}/projects/${projectInfo.scriptureForgeProjectId}/draft-generation)`,
                          },
                        },
                        containers: {
              ...
              >
                {isDraftGenerating ? 'View old draft' : 'View draft'}
              </Button>
            </div>
          );
        ...
        return isDraftGenerating ? (
          ...
          >
            {projectInfo.draftInfo.draftSetupState === 'noFinishedDraft' ? 'Generate' : 'Sign up'}
          </ExternalLinkButton>
        );
      }
      default:
    ...
    <Table stickyHeader>
      <TableHeader stickyHeader>
        <TableRow>
          ...
          {buildTableHead('draftSetupState', 'Status')}
