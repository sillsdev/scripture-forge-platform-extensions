...
import { IRpcMethodRegistrar } from './rpc.interface';
import {
  ConnectionStatus,
  createErrorResponse,
  createRequest,
...
import { bindClassMethods, SerializedRequestType } from './util';
  ...
  async connect(localEventHandler: EventHandler): Promise<boolean> {
    return this.connectionMutex.runExclusive(async () => {
      ...
      try {
        ...
        this.ws = new WebSocket(`ws://localhost:${this.webSocketPort}`);
